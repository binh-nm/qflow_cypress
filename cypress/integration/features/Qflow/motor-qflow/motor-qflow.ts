...
import { baseUrl, motorQuestionPageUrl } from '../../../../constants';
import { fillFieldWithValue, getFieldType, checkPackages, confirmContactInfo} from '../../../../utils';
import { cases } from '../../../../fixtures/motor-qflow-questions.json';

Before({ tags: '@english' }, () => {
    cy.visit(`${baseUrl}en/${motorQuestionPageUrl}`, {
      auth: {
        username: '1423',
        password: '1423',
      },
  ...
  Before({ tags: '@thai' }, () => {
    cy.reload();
    cy.visit(`${baseUrl}${motorQuestionPageUrl}`, {
      auth: {
        username: '1423',
        password: '1423',
      },
  ...
  And(
    'I click button to see All Packages', () => {
        cy.get('button').contains('See all packages').click()
    }
  );
